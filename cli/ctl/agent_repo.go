	...
	create := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if strings.HasSuffix(image, ".exe") {
				if versionImage == "" {
					fmt.Printf("version-image must be set when uploading a window image")
					return
				}
				...
				printutil.ErrorfWithColor("make sure %s and %s have the same version", image, versionImage)
			}
