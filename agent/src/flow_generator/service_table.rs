    ...
    pub fn get_ebpf_tcp_score(
        ...
        match (flow_src_key, flow_dst_key) {
            (ServiceKey::V4(flow_src_key), ServiceKey::V4(flow_dst_key)) => {
                ...
                if let Some(s) = self.ipv4.get(&flow_dst_key) {
                    score = *s;
                } else if let Some(s) = self.ipv4.get(&flow_src_key) {
                    // if get score from flow_src_key, it indicate that the packet maybe disorder, the flow should be reverse
                    score = *s;
                    need_reverse = true;
                } else if socket_role > 0 {
            ...
            (ServiceKey::V6(flow_src_key), ServiceKey::V6(flow_dst_key)) => {
                if let Some(s) = self.ipv6.get(&flow_dst_key) {
                    score = *s;
