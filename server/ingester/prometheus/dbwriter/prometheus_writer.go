...
func (w *PrometheusWriter) InitTable() error {
	...
	w.getOrCreateCkwriter(&Prometheus{})
	return err
}

func (w *PrometheusWriter) getOrCreateCkwriter(s *Prometheus) (*ckwriter.CKWriter, error) {
	// aligned by APP_LABEL_COLUMN_PER_INCREMENT
	appLabelCount := (len(s.AppLabelValueIDs) + APP_LABEL_COLUMN_PER_INCREMENT - 1) / APP_LABEL_COLUMN_PER_INCREMENT * APP_LABEL_COLUMN_PER_INCREMENT
	if appLabelCount < APP_LABEL_COLUMN_PER_INCREMENT {
		appLabelCount = APP_LABEL_COLUMN_PER_INCREMENT
	}
...
func (w *PrometheusWriter) addAppLabelColumns(conn common.DBs, startIndex, endIndex int) error {
	for i := startIndex; i < endIndex; i++ {
		for _, table := range []string{PROMETHEUS_TABLE + "_local", PROMETHEUS_TABLE} {
			_, err := conn.Exec(fmt.Sprintf("ALTER TABLE %s.`%s_local` ADD COLUNN app_lable_value_id_%d %s",
...
func (w *PrometheusWriter) getActualAppLabelColumnCount() (int, error) {
	sql := fmt.Sprintf("SELECT count(*) FROM system.columns where database='%s' and table='%s' and name like '%%app_lable_value%%'", PROMETHEUS_DB, PROMETHEUS_TABLE)
	log.Info(sql)
...
func NewPrometheusWriter(
	...
	common.RegisterCountableForIngester("prometheus_writer", writer, stats.OptionStatTags{"msg": msgType.String(), "decoder_index": strconv.Itoa(decoderIndex)})
	return writer, nil
}
