...
const (
	...
	APP_LABEL_COLUMN_PER_INCREMENT = 4
)
...
func (m *Prometheus) Columns(appLabelColumnCount int) []*ckdb.Column {
	...
	columns = append(columns,
		ckdb.NewColumn("metrics_id", ckdb.UInt32).SetComment("encoded ID of the metric name"),
		ckdb.NewColumn("target_id", ckdb.UInt32).SetComment("the encoded ID of the monitoring target"),
	)
...
func (m *Prometheus) GenCKTable(cluster, storagePolicy string, ttl int, coldStorage *ckdb.ColdStorage, appLabelColumnCount int) *ckdb.Table {
	...
	// order key
	orderKeys := []string{}
	orderKeys = append(orderKeys, "metrics_id")
