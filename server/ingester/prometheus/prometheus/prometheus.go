...
func NewPrometheus(config *config.Config, recv *receiver.Receiver, platformDataManager *grpc.PlatformDataManager) (*Prometheus, error) {
	...
	prometheusLabelTable := decoder.NewPrometheusLabelTable(config.Base.ControllerIPs, int(config.Base.ControllerPort), config.GrpcBufferSize)
	...
	for i := 0; i < queueCount; i++ {
		...
		if i == 0 {
			debug.ServerRegisterSimple(CMD_PROMETHEUS, platformDatas[i])
		}
		if err != nil {
			return nil, err
		}
		metricsWriter, err := dbwriter.NewPrometheusWriter(i, msgType, dbwriter.PROMETHEUS_DB, config)
		if err != nil {
			return nil, err
		}
		...
		slowPlatformDatas[i], err = platformDataManager.NewPlatformInfoTable(false, msgType.String()+"-"+strconv.Itoa(i))
...
func (m *Prometheus) Start() {
	m.prometheusLabelTable.RequesteAllLabelIDs()
