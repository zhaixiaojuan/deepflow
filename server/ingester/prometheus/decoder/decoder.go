...
type Counter struct {
	...
	SlowMetrics int64 `statsd:"slow-metrics"`
	TimeSeries  int64 `statsd:"time-series"` // count the number of TimeSeries (not Samples)
}
...
func (b *BuildPrometheus) TimeSeriesToStore(vtapID uint16, ts *prompb.TimeSeries) (bool, error) {
	...
	targetID, ok := b.labelTable.QueryTargetID(metricID, jobID, instanceID)
	if !ok {
		if noTarget%1000 == 0 {
		...
		noTarget++
		return false, fmt.Errorf("can't get prometheus targetID, metricName:%s(%d),job:%s(%d),instance:%s(%d)",
			metricName, metricID, job, jobID, instance, instanceID)
	}
	b.appLabelValueIDsBuffer = append(b.appLabelValueIDsBuffer, make([]uint32, maxColumnIndex+1)...)

	for i, index := range b.labelColumnIndexsBuffer {
		// index==0 is target label
		if index == 0 {
			continue
		}
