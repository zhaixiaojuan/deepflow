...
type DHCPPort struct {
	UpdaterBase[cloudmodel.DHCPPort, mysql.DHCPPort, *diffbase.DHCPPort, *message.DHCPPortAdd, message.DHCPPortAdd, *message.DHCPPortUpdate, message.DHCPPortUpdate, *message.DHCPPortFieldsUpdate, message.DHCPPortFieldsUpdate, *message.DHCPPortDelete, message.DHCPPortDelete]
}

func NewDHCPPort(wholeCache *cache.Cache, cloudData []cloudmodel.DHCPPort) *DHCPPort {
	updater := &DHCPPort{
		newUpdaterBase[cloudmodel.DHCPPort, mysql.DHCPPort, *diffbase.DHCPPort, *message.DHCPPortAdd, message.DHCPPortAdd, *message.DHCPPortUpdate, message.DHCPPortUpdate, *message.DHCPPortFieldsUpdate, message.DHCPPortFieldsUpdate, *message.DHCPPortDelete](
...
func (p *DHCPPort) generateUpdateInfo(diffBase *diffbase.DHCPPort, cloudItem *cloudmodel.DHCPPort) (*message.DHCPPortFieldsUpdate, map[string]interface{}, bool) {
	...
	if diffBase.RegionLcuuid != cloudItem.RegionLcuuid {
		...
		structInfo.RegionLcuuid.SetNew(cloudItem.RegionLcuuid)
	}
	if diffBase.AZLcuuid != cloudItem.AZLcuuid {
		mapInfo["az"] = cloudItem.AZLcuuid
		structInfo.AZLcuuid.SetNew(cloudItem.AZLcuuid)
	}
