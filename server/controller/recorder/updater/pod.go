...
type Pod struct {
	UpdaterBase[cloudmodel.Pod, mysql.Pod, *diffbase.Pod, *message.PodAdd, message.PodAdd, *message.PodUpdate, message.PodUpdate, *message.PodFieldsUpdate, message.PodFieldsUpdate, *message.PodDelete, message.PodDelete]
}

func NewPod(wholeCache *cache.Cache, cloudData []cloudmodel.Pod) *Pod {
	updater := &Pod{
		newUpdaterBase[cloudmodel.Pod, mysql.Pod, *diffbase.Pod, *message.PodAdd, message.PodAdd, *message.PodUpdate, message.PodUpdate, *message.PodFieldsUpdate, message.PodFieldsUpdate, *message.PodDelete, message.PodDelete](
...
func (p *Pod) generateDBItemToAdd(cloudItem *cloudmodel.Pod) (*mysql.Pod, bool) {
	...
	if !exists {
		log.Errorf(resourceAForResourceBNotFound(
			ctrlrcommon.RESOURCE_TYPE_POD_NODE_EN, cloudItem.PodNodeLcuuid,
			ctrlrcommon.RESOURCE_TYPE_POD_EN, cloudItem.Lcuuid,
		))
		return nil, false
	}
