...
type UpdaterBase[CT constraint.CloudModel, MT constraint.MySQLModel, BT constraint.DiffBase, MAPT msg.AddPtr[MAT], MAT msg.Add, MUPT msg.UpdatePtr[MUT], MUT msg.Update, MFUPT msg.FieldsUpdatePtr[MFUT], MFUT msg.FieldsUpdate, MDPT msg.DeletePtr[MDT], MDT msg.Delete] struct {
	resourceType string
	...
	pubsub    pubsub.SingleResourcePubSub[MAPT, MAT, MUPT, MUT, MFUPT, MFUT, MDPT, MDT] // 用于发布订阅的消息中心
...
func newUpdaterBase[CT constraint.CloudModel, MT constraint.MySQLModel, BT constraint.DiffBase, MAPT msg.AddPtr[MAT], MAT msg.Add, MUPT msg.UpdatePtr[MUT], MUT msg.Update, MFUPT msg.FieldsUpdatePtr[MFUT], MFUT msg.FieldsUpdate, MDPT msg.DeletePtr[MDT], MDT msg.Delete](
	resourceType string,
	cache *cache.Cache,
	dbOperator db.Operator[MT],
...
func (u *UpdaterBase[CT, MT, BT, MAPT, MAT, MUPT, MUT, MFUPT, MFUT, MDPT, MDT]) initPubSub() {
	...
	u.pubsub = ps.(pubsub.SingleResourcePubSub[MAPT, MAT, MUPT, MUT, MFUPT, MFUT, MDPT, MDT])
}
...
func (u *UpdaterBase[CT, MT, BT, MAPT, MAT, MUPT, MUT, MFUPT, MFUT, MDPT, MDT]) update(cloudItem *CT, diffBase BT, info map[string]interface{}, fields MFUPT) {
	if dbItem, ok := u.dbOperator.Update(diffBase.GetLcuuid(), info); ok {
		...
		if u.pubsub != nil {
			u.pubsub.PublishUpdate(msgData)
		}
		u.Changed = true
	}
}
...
func (u *UpdaterBase[CT, MT, BT, MAPT, MAT, MUPT, MUT, MFUPT, MFUT, MDPT, MDT]) deletePage(lcuuids []string) {
	if dbItems, ok := u.dbOperator.DeleteBatch(lcuuids); ok {
		...
		if u.pubsub != nil {
			u.pubsub.PublishDelete(msgData)
		}
		u.Changed = true
	}
}
